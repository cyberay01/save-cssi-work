...
'''
def find_most_enrolled(en_list):
    student_list = []
    counter_list = []
    for enroll in en_list:
        included = False
        for i in range(len(student_list)):
            if student_list[i] == enroll.student.get():
                included = True
                counter_list[i] += 1
                print(student_list[i].first_name + ": " + str(counter_list[i]))
        if included == False:
            student_list.append(enroll.student.get())
            counter_list.append(1)
            print(student_list[len(student_list) - 1].first_name + ": " + str(counter_list[len(counter_list) - 1]))
    return student_list[counter_list.index(max(counter_list))]

def find_most_enrolled(en_list):
    student_list = []
    for enroll in en_list:
        student_list.append(enroll.student.get().student_id)
    student_dict = {x: student_list.count(x) for x in student_list}
    key_max = max(student_dict.keys(), key=(lambda k: student_dict[k]))
    return hogwarts_models.Student.query(hogwarts_models.Student.student_id == key_max).fetch()[0]


 def find_most_enrolled(en_list):     
    course_count = {}
    for enroll in en_list:
        count = course_count.get(enroll.student, 0)
        course_count[enroll.student] = count + 1
    key_max = max(course_count.keys(), key=(lambda k: course_count[k]))
    return key_max.get().first_name
